...
const HERE = import.meta.url.startsWith('file:') ? new URL('.', import.meta.url).pathname : process.cwd();
...
async function downloadImages() {
  ...
  const imagePromises = Object.entries(map).map(async ([key, product]) => {
    if (product.image) {
      const ext = product.image.split('?')[0].split('.').pop();
      const imagePath = join(DOWNLOAD_PATH, `${key}.${ext}`);
      if ((!await exists(imagePath)) || !product.image.startsWith('http')) {
        try {
          const response = await fetch(product.image);
          if (!response.ok) {
            throw new Error(`Failed to download image for ${key}: ${response.statusText}`);
          }
          const buffer = await response.arrayBuffer();
          await writeFile(imagePath, Buffer.from(buffer));
          console.log(`Image for ${key} saved to ${imagePath}`);
        } catch(error) {
          console.error(`Error downloading image for ${key} (${product.image}) :`, error);
        }
      } else {
        console.log(`Image for ${key} already exists at ${imagePath}`);
      }
    }
  });
...
async function transformImages() {
  ...
  const transformPromises = images.map(async (image) => {
    ...
    await sharp(imagePath).webp({ quality: 80 }).toFile(imagePath.replace(/\.\w+$/, '.webp'))
