...
<div class="flex w-full flex-col gap-1">
  {#if placeholder}
    <label for={name} class="text-muted text-sm font-medium">{placeholder}</label>
  {/if}
  <div class="relative">
    <input
      {name}
      class={cn(
        'border-border focus:ring-primary w-full rounded border p-2 focus:ring-2 focus:outline-none',
        className
      )}
      {placeholder}
      type={innerType}
