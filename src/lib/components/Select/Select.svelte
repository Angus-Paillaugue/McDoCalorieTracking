<script lang="ts">
  ...
  const baseClasses =
    'block bg-background text-foreground text-sm min-w-[40px] relative font-medium px-2 py-1 rounded font-sans focus:outline-hidden border border-border outline-hidden focus:ring-2 transition-all ring-primary flex flex-row gap-2 items-center';
...
>
  ...
  <div
    ...
    transition:slide
  >
    <div class="border-border bg-background flex flex-col overflow-hidden rounded border">
      {@render children?.()}
    </div>
  </div>
</div>
